  ...
  return (
    <div className="navbar bg-base-100">
      <div className="flex-1">
        ...
        <Link className=" text-xl font-bold">Looks-To-Do</Link>
      </div>
      <div className=" nav-link mr-12 lg:block md:block hidden">

        {
          user ? <> <Link className="ml-4" to="/myTask">My Task</Link>
      ...
      {
        user && <Link to='/' onClick={handleLogOut} className="mr-6 border rounded-full bg-gray-400 cursor-pointer p-0.5 hover:bg-red-400">
      ...
      <div className="flex-none gap-2">
        <div className="dropdown dropdown-end">
          <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
            ...
            {
              user || user?.photoURL ? <><div className="w-10 rounded-full">
              <img src={user?.photoURL} alt="user-img" title={user?.displayName} />
            </div> </> : <><img className="w-10" src={profileIcon} alt="" /></>
            }
